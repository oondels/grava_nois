  ...
  >
    <div class="mx-auto w-full max-w-6xl px-3 md:px-6 h-16 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-2">
        ...
        <span class="relative">
          <HeaderLink to="#login" label="Login" aria-disabled="true" class="opacity-50 pointer-events-none select-none" />
          <span class="absolute -right-3 -top-2 text-[10px] px-1.5 py-0.5 rounded-full bg-amber-500/90 text-black font-semibold">breve</span>
        </span>
      </nav>
    </div>
  </header>
...
function handleLinkClick() { if (window.innerWidth <= 660) close(); }
function onResize() { if (window.innerWidth > 660 && isOpen.value) close(); }
...
const HeaderLink = defineComponent({
  ...
  setup(props) {
    ...
    return () =>
      h(
        RouterLink,
        { to: props.to, class: `${baseLink} ${isActive() ? 'text-green-600 dark:text-green-400' : 'text-white-800 dark:text-white-100'}`, "aria-current": isActive() ? "page" : null },
        {
          default: () => [
            props.label,
            h('span', {
